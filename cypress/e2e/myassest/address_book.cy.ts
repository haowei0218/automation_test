...
import MyassetsMainPage from "../../page/Myasset";
describe("address book", () => {
  beforeEach(() => {
    const Login = new MyassetsMainPage();
    Login.login("myademonumber8@gmail.com", "Myatesting#");
    cy.viewport(1980, 1080);
    cy.get(".MuiAvatar-root").click();
    cy.get(":nth-child(5) > .jss93").click();
    cy.get(".MuiTableCell-root > .MuiToolbar-root")
      .eq(0)
      .find("select")
      .select("100");
  });
  context("address book button functionanl", () => {
    ...
    it.only("Fill out the contact information page", () => {
      ...
      const createContact = new MyassetsMainPage();
      createContact.ContactInfoPage();
    });
    it("Update contact information", () => {
      const firstName = "Update" + random(100);
      cy.get(
        ":nth-child(14) > .jss488 > .MuiGrid-container > :nth-child(1) > .MuiButtonBase-root > .MuiIconButton-label > .MuiBox-root > .MuiSvgIcon-root"
      ).click();
    ...
    it("keep Editing button", () => {
      cy.get('[data-cy="add-btn"]').click("center");
      cy.get(
        ".MuiGrid-justify-xs-center > :nth-child(2) > .MuiButtonBase-root > .MuiIconButton-label > .MuiSvgIcon-root > path"
      ).click("center");
  ...
  context("Contacts information ", () => {
    ...
    it("#address", () => {
      ...
      if (
        cy
          .get('.jss741 > :nth-child(1) > .MuiList-root > [tabindex="0"]')
          .should("exist")
          .contains("New York")
      ) {
        console.log("pass");
    ...
    it("Country", () => {
      cy.get('[data-cy="add-btn"]').click();
      cy.get(
        ".jss749 > :nth-child(1) > .MuiAutocomplete-root > .MuiFormControl-root > .MuiInputBase-root"
      ).click();
      cy.get("#country-option-0").click();
    });
    it.only("assets per page button", () => {
      const assetsPerPage = new MyassetsMainPage();
      assetsPerPage.perpage;
    });
    it.only("delete button", () => {
      cy.get(
        ":nth-child(1) > .jss488 > .MuiGrid-container > :nth-child(2) > .MuiButtonBase-root > .MuiIconButton-label > .MuiBox-root"
      ).click();
