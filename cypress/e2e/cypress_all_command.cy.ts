...
describe("cypress all commands", () => {
  ...
  context("select function", () => {
    ...
    it(".select(value,options)", () => {
      cy.get("select")
        .eq(1)
        .select("bg", { force: true })
        .should("contain.text", "北京");
    });
  });
  ...
  context("獲取全局變量", () => {
    ...
    it.only("reload example", () => {
      ...
      cy.get("#logo > a >img");
      cy.reload();
      cy.get(".nav-item > :nth-child(6) > :nth-child(1)")
        .click()
        .should("exist");
      cy.reload(true);
    });
  ...
  context("type function", () => {
    ...
    it("帶有單個參數的傳遞", () => {
      cy.get(".single_option").type("selectall", {
        parseSpecialCharSequences: false,
      });
    });
    it("帶有多個參數的傳遞", () => {
      cy.get(".single_option").type("1234", {
        log: false,
        parseSpecialCharSequences: false,
      });
    });
  ...
  context("get 請求方法", () => {
    ...
    it("request 替代 visit", () => {
      cy.request("http://localhost://7077/login")
        .its("body")
  ...
  context("each 命令詳解", () => {
    ...
    it("範例1", () => {
      cy.get(".connectors-each-ul>li").each(
        (item: JQuery<HTMLElement>, index: number, $list: HTMLElement[]) => {
